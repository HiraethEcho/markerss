...
- sdd 增强层: 见下方 `## SDD Workflow Layer` — 命令速查 / 访谈钩子 / 审查钩子 / agent-skills 联动

## SDD Workflow Layer (sdd enhancement)

This project layers the sdd workflow on top of the lightspec three-stage core. Core flow is unchanged; extra hooks:

### Routing entry

- Unsure what's next → read `skills/app-sdd/SKILL.md` (default/full) or `skills/app-lite-sdd/SKILL.md` (lite) and route by stage
- New session / interrupted work → `/pickup` (reads `lightspec list` + `changes/<id>/tasks.md` pending items, outputs next step + suggested skill)
- New project init → `/sdd-init` (copies this file + writes SPEC.md + PLAN.md)
...
### Requirements clarification hook (before proposal)

- Vague request → run discovery first: `discovery` + `idea-refine`, then the spec
- Draft proposal done → `plan-reviewer` review, then `lightspec validate --strict`

### Apply hooks (during implementation)

- Coding → `building` (plan-driven mode, thin slices, ponytail)
- Testing → `test-driven-development` + acceptance validation from the plan
- Task self-review → `reviewing` (five-axis + fresh-perspective + criteria-walk)
- Security-sensitive → `security-and-hardening`
- All tasks complete → `code-reviewer` full pass + `lightspec validate --strict`, then archive

### Agent-skills linkage

- This workflow invokes the merged sdd skills in `skills/`, aligned with the lightspec phases (proposal → spec-proposal/spec-driven-development, apply → building/tdd, review → reviewing)
