...
pub fn export_opml(file: &File) -> String {
    ...
    for cat in file.categories() {
        out.push_str(&format!("<outline text=\"{cat}\" title=\"{cat}\">\n"));
        for f in file.by_category(&cat) {
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(e)) => {
                ...
                if name == "outline" {
                    ...
                    if let Some(url) = url {
                        feeds.push(make_feed(url, title, &stack));
...
fn make_feed(url: String, title: Option<String>, tags: &[String]) -> Feed {
