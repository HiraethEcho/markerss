...
- Keys: `o` browser / `e` export (rename prompt, default path prefilled) / `a` toggle-read / `A` mark-all-read / `N` new feed / `d` delete (x2) / `M` rename (feed title / category / tag) / `F` favourite in nav, fullscreen in article / `L` read-later, `S` saved (list+article) / `t` nav preset cycle / `r` partial refresh (current scope feeds) / `R` refresh all / `i`/`x` OPML / `Q` quit / `?` help
